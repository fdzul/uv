---
title: "Predicción de la abundancia del vector del dengue usando Algoritmos de Machine Learning"
format: 
  html:
      self-contained: true
      theme: flatly
...
La base de datos tiene cuatro variables, las coordenadas geográficas (x, y), la semana y la variable de interés (eggs). Las coordenadas geográficas tienen un sistema de referencia (CRS) con código EPSG 4326.

```{r echo=FALSE}
load("~/Library/CloudStorage/Dropbox/dataset/automatic_read_ovitraps/eggs_data.RData")
gtExtras::gt_plt_summary(eggs_data)
```
...
2. Reproducibilidad.
3. Eficiencia del algoritmo mediante las siguientes métricas: RMSE, MAE y R²
